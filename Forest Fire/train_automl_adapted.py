...
def plot_lgb_training_curve(eval_results, save_path="training_curve.png"):
    """
    eval_results → LightGBM evals_result_ dict
    Example:
    model.fit(..., eval_set=[(...), (...)], eval_names=['train','valid'], eval_metric=['auc','binary_logloss'])
    """
    ...
    # ---- AUC curves ----
    ax1.plot(iters, train_auc, label="train-auc", color="blue")
    ax1.plot(iters, valid_auc, label="valid-auc", color="orange")
    ax1.set_ylabel("AUC")
    ...
    # ---- Logloss curves ----
    ax2 = ax1.twinx()
    ax2.plot(iters, train_loss, label="train-binary_logloss", color="green")
    ax2.plot(iters, valid_loss, label="valid-binary_logloss", color="red")
    ax2.set_ylabel("Binary Logloss")
    ...
    plt.title("Training Curve (classifier)")
    plt.tight_layout()
...
def tune_lightgbm_optuna(X, y, n_trials=30, random_state=42):
    import optuna
    import lightgbm as lgb
    from sklearn.metrics import average_precision_score
    from sklearn.model_selection import StratifiedKFold
    ...
    def objective(trial):
        params = {
            ...
            "verbose": -1,
            "seed": random_state,
            "num_leaves": trial.suggest_int("num_leaves", 16, 120),
        ...
        for tr, te in cv.split(X, y):
            ...
            score = average_precision_score(yv, probs)
            scores.append(score)
...
def run_optuna_tuning(X_train, y_train, X_valid, y_valid, is_classifier=True, n_trials=20):
    ...
    def objective(trial):
        ...
        if model_name == "dl":
            ...
            rec = recall_score(y_valid, (val_probs >= 0.2).astype(int))
            score = pr_auc + 0.3 * rec     # recall enforcement added
        ...
        # ================= TREE MODELS =================
            ...
            if is_classifier:
                ...
                # === NEW – recall enforcement ===
                rec = recall_score(y_valid, (probs >= 0.2).astype(int))
                score = pr_auc + 0.8 * rec
            else:
...
def _evaluate_classifier(best_clf, data, outdir_zone, zone_id, meta_info):
    ...
    ✔ Auto-optuna tuning for LightGBM baseline
    ✔ ROC + PR curves
    ...
    y_train = data["y_tr_cls"]

    X_test  = data["X_test"]
    y_true  = data["y_te_cls"]

    # ==================================================================
    # 🔥 If LightGBM model → apply Optuna tuning
    # ==================================================================
    if "LGBM" in type(best_clf).__name__:
        print(f"\n⚡ Optuna tuning activated for Zone {zone_id} LightGBM...")
        tuned_model, tuned_params, tuned_pr_auc = tune_lightgbm_optuna(X_train, y_train)

        best_clf = tuned_model
        meta_info["params"] = tuned_params
        meta_info["pr_auc"] = tuned_pr_auc
        print("✔ Optuna tuning done — best params updated.")
    ...
    try:
        ...
        metrics_dict["auc"] = float(auc_score)
    ...
    best_f2 = max(
        ...
        for t in np.linspace(0.01, 0.60, 40)
    )
    ...
    # ---------------- Store final metrics ----------------
    final_metrics = {
        "best_model": type(best_clf).__name__,
    ...
    json.dump(final_metrics, open(os.path.join(outdir_zone, "classifier_meta.json"), "w"), indent=2)

    return best_thr, final_metrics, y_true.tolist(), probs.tolist()
...
def train_zone(df_zone, outdir_zone, zone_id, args=None, min_samples=50):
    ...
    try:
        entry_old = registry.get(zone_id, {})
        missed_idx = entry_old.get("missed_fires_pool", [])

        if isinstance(missed_idx, list) and len(missed_idx) > 0:
            print(f"[ACTIVE LEARNING] boosting missed fires = {len(missed_idx)}")

            boost = df_zone.iloc[missed_idx].copy()
            df_zone = pd.concat([df_zone] + [boost]*10, ignore_index=True)

            # Recompute preprocessing
            data = _prepare_data(df_zone)

            print("[i] Rebalanced shapes:", data["X_train"].shape)
    except Exception as e:
        print("[!] Active learning injection failed:", e)
    ...
    # create FN pool storage if first run
    missed_idx = []
    try:
        # FN extraction from test fold
        y_arr = np.array(y_true)
        p_arr = np.array(y_prob)
        missed_idx = [i for i,(yt,pr) in enumerate(zip(y_arr,p_arr)) if yt==1 and pr < best_thr]
    except:
        pass
...
def train_all_zones(tasks, registry, parallel=False):
    ...
    if not parallel:
        ...
        for params in tasks:
            ...
            old = registry.get(zone_id, {})

            # Carry forward previous FN memory
            prev_pool = old.get("missed_fires_pool", [])

            new_pool = result.get("missed_fires_pool", [])

            # merge & deduplicate (cap size if needed)
            final_pool = list(set(prev_pool + new_pool))[:500]

            result["missed_fires_pool"] = final_pool
